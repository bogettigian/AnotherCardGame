...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] ConnectedTextNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is ConnectedTextNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (ConnectedTextNetworkObject.Length > 0 && ConnectedTextNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(ConnectedTextNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<ConnectedTextBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateConnectedText instead, its shorter and easier to type out ;)")]
		public ConnectedTextBehavior InstantiateConnectedTextNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(ConnectedTextNetworkObject[index]);
			var netBehavior = go.GetComponent<ConnectedTextBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<ConnectedTextBehavior>().networkObject = (ConnectedTextNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of ConnectedText
		/// </summary>
		/// <returns>
		/// A local instance of ConnectedTextBehavior
		/// </returns>
		/// <param name="index">The index of the ConnectedText prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public ConnectedTextBehavior InstantiateConnectedText(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(ConnectedTextNetworkObject[index]);
			var netBehavior = go.GetComponent<ConnectedTextBehavior>();
